...
const BASE_TARGETS = [
  ...
  { id: 'aider-cli', name: 'Aider', kind: 'cli', command: 'aider', routing: 'compatible', modelCommand: '/model <name>', modelSwitch: 'Native /model <name> command; routed catalog printed at launch', candidates: ['%USERPROFILE%\\.local\\bin\\aider.exe'], description: 'OpenAI-compatible model and endpoint passed at launch. Aider receives the native terminal directly so typing remains responsive.' },
...
function launchTerminal(executable, args, options = {}) {
  if (process.platform === 'win32') {
    const command = [
      ...
      ...(options.banner || []).map((line) => `Write-Host ${quotePowerShell(line)} -ForegroundColor DarkGray`),
  ...
  if (banner.length) {
    const script = [
      ...banner.map((line) => `printf '%s\\n' ${shellQuote(line)}`),
      `exec ${[executable, ...args].map(shellQuote).join(' ')}`
    ].join('\n');
    const child = spawn('/bin/sh', ['-c', script], { cwd: options.cwd, env: options.env, detached: true, stdio: 'ignore' });
    child.unref();
    return;
  }
...
class TargetLauncher {
  ...
  async launch({ targetId, provider, apiKey, model, models = [model], workspace, localGatewayBaseUrl }) {
    ...
    if (target.id === 'aider-cli') {
      ...
      // directly. Its native /model command switches live without a relaunch;
      // the banner lists the routed catalog because Aider's own /models
      // command only searches built-in LiteLLM names.
      const routed = [...new Set([model, ...models].filter(Boolean))];
      const listed = routed.slice(0, 12);
      const banner = [
        `CodeSwitchboard routed catalog (${routed.length} models). Switch with /model openai/<name>:`,
        ...listed.map((id) => `  /model openai/${id}`),
        ...(routed.length > listed.length ? [`  ... ${routed.length - listed.length} more, same pattern`] : []),
        "Aider's /models only searches built-in LiteLLM names; use /model with a name above."
      ];
      ...
      return { message: `Aider launched with ${model}. Switch models inside Aider with /model openai/<name>.` };
    }
